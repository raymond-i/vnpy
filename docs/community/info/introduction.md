...
若你刚开始学习量化和编程，希望先建立业务概念再动手：

- **对量化和 VeighNa 概念还不熟悉**：可先阅读 [量化与 VeighNa 业务概念速览](quant_business_concepts.md)，了解量化交易、CTA、回测与实盘、行情与合约等在 VeighNa 中的对应关系。
- **已安装环境，想马上跑通一个策略并完成回测、加载**：请按 [新同学编程入门与实操](newcomer_quickstart.md) 一步步操作，从最小 CTA 策略到回测、实盘加载均有可执行步骤与示例代码。
